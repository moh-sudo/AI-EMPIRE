...
def _section(title: str, items: list, empty_message: str, formatter: Callable[[Any], str] = str, limit: int = 15) -> str:
...
def run_daily_briefing() -> dict:
    ...
    try:
        ...
        sections.append(_section(
            ...
            formatter=lambda r: f"booking {r.get('booking_id')} -> " + (f"matched worker {r['matched_worker_id']}" if r.get("matched_worker_id") else "NO ELIGIBLE WORKER FOUND"),
    ...
    try:
        ...
        sections.append(_section(
            "FINANCIAL OPERATIONS", escalations,
    ...
    try:
        ...
        parts = []
        if r["dispute_triage"]:
            parts.append(_section("  Disputes needing attention", r["dispute_triage"], "", formatter=lambda t: f"{t.dispute_id}: {t.priority} ({t.reason})"))
        if r["fraud_signals"]:
        ...
        sections.append("TRUST & SAFETY:\n" + "\n".join(parts) if parts else "TRUST & SAFETY: clean -- no disputes at risk, no fraud signals, no KYC re-verifications due.")
    except Exception as e:
    ...
    try:
        ...
        sections.append(_section(
            "MARKETPLACE INTELLIGENCE", bottlenecks,
            "no service bottlenecks detected.",
            formatter=lambda s: f"{s.service}: {s.open_booking_count} open booking(s) vs {s.eligible_worker_count} eligible worker(s) (ratio {s.ratio:.1f})",
        ))
    except Exception as e:
    ...
    try:
        ...
        sections.append(_section(
            "CUSTOMER SUPPORT", breached,
            f"{len(queue)} ticket(s) in queue, none breaching SLA.",
            formatter=lambda t: f"ticket {t.ticket_id}: {t.priority} priority, open {t.hours_open:.1f}h ({t.sla_status})",
        ))
    except Exception as e:
    ...
    summary = f"Fixera Division Daily Briefing (CEO/Lead Agent)\n{generated_at.strftime('%Y-%m-%d %H:%M UTC')}\n\n" + "\n\n".join(sections)
