...
- **Access pattern 1 (audit and the central `api_gateway` only, as of 2026-08-10):** `shared/db.py`'s `get_client()`, authenticated with the all-powerful `SUPABASE_SERVICE_KEY`, bypassing Row-Level Security entirely. Deferred deliberately for audit (it needs genuine cross-division, unfiltered reads on several tables — a different access shape than the other divisions, not yet designed) and for the multi-writer tables (`audit_vault`, `routing_logs`, `agent_registry` — shared across divisions, no single exclusive claim fits without more design work).
- **Access pattern 2 (Systems & Automation, Fixera, Forex, Personal & Education, Learning, and RII):** a self-signed JWT carrying a custom `app_role: "<division>_agent"` claim, checked by real RLS policies. The JWT-minting mechanism itself lives in `shared/scoped_db.py`'s `get_scoped_client(app_role)` — generic, shared infra (extracted 2026-08-10 from `shared/systems_db_connector.py`, which had zero Systems-specific logic in its own client-minting code). Systems remains scoped to `circuit_breakers`/`audit_vault` only (`0010_systems_agent_rls_jwt.sql`), proven with real negative tests (2026-08-06) that it genuinely cannot read `agent_registry` or `routing_logs`. The other 5 divisions were scoped 2026-08-10 (`0014_five_divisions_rls_jwt.sql`) to exactly what each division's own code actually does — confirmed via a real audit of every `.table()` call across all 6 divisions, not guessed: fixera/forex/personal/learning/rii each get `memory_experience`/`memory_knowledge` INSERT scoped to their own `division` value (forex additionally gets SELECT on its own rows, the only one of the 5 that reads it today), plus SELECT/INSERT/UPDATE on their own division-exclusive tables (`personal_habits`+`personal_habit_completions`, `learning_cards`+`learning_card_links`+`curriculum_phases`+`curriculum_subjects`, `rii_watchtowers`). `shared/memory/experience.py`'s `add_experience()` and `shared/memory/knowledge.py`'s `add_knowledge()` (the shared write-path both `_memory_helpers.py` and `srs.py`/`habit_tracker.py`/etc. call into) both gained an optional `client` parameter defaulting to the old blanket client — adding it never changed behavior for any caller not yet passing a scoped one.
...
- **~~Access pattern 1 (the blanket service-role key) is still what 5 of 6 divisions use~~ — RESOLVED for 5 of them, 2026-08-10.** Fixera, Forex, Personal & Education, Learning, and RII now use scoped RLS+JWT (`0014_five_divisions_rls_jwt.sql`), matching Systems & Automation's already-proven pattern. **Still on the blanket key:** audit (needs genuine cross-division, unfiltered reads — a different access shape, deliberately deferred rather than designed hastily) and the multi-writer tables it and the central `api_gateway` touch (`audit_vault`, `routing_logs`, `agent_registry` — no single division's claim fits, needs its own design). Migration written and code changes locally verified (full lint pass, full test suite green); **not yet run against production Supabase** — that's Mohamed's step, same as every migration in this project. Division functionality against the real scoped credentials still needs live verification once the migration is applied.
