...
Voice-note handling only exists on Learning's Telegram bot today; every other division's bot is text-only, and `voice_capture.py`'s CLI entry point still requires manually running it — no wake-word/always-listening trigger exists yet (see "Wake word + agent identities," next). Real phone calling (Twilio) is a separate, larger, not-yet-started decision.

### Wake word + agent identities (not yet built)

Mohamed wants two things layered on top of the voice pipeline, decided 2026-08-10, not yet built:

1. **Hands-free activation:** clap to arm a short listening window, then say "wake up" to actually trigger a capture — chosen over clap-only (too many false positives from any loud noise) or phrase-only (would mean running some form of speech detection continuously). Chosen to run as an **always-on background service**, not something manually started each time — meaning it needs the same "must survive/restart across reboots" treatment as n8n and the division servers already need (see "Known gaps," nothing persists across a restart), plus real consideration of the project's own Energy-Aware Computing standard (`CONTEXT.md`'s Operational Efficiency Standard) before it's built as something that runs 24/7.
2. **Named identities:** each of the 6 division "CEOs" and a "supreme boss" overseeing all of them should have actual names, not just division labels — a personality/branding layer, names still to be chosen.
