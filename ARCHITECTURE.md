...
**Text-to-speech is also real now**, via `pyttsx3` (Windows SAPI5, offline, no model download) in `shared/voice/text_to_speech.py` — much lighter than STT since there's no native ML dependency chain. Live-verified: synthesized speech played back audibly through real speakers. Voice-note handling only exists on Learning's Telegram bot today; every other division's bot is text-only. Real phone calling (Twilio) is a separate, larger, not-yet-started decision.
